...
### Custom Model Servers (provider `custom`, shipped 2026-07-09)
User-registered OpenAI-compatible LAN servers (llama.cpp/llama-swap/vLLM/Ollama) as a first-class chat provider across Portal, Android, cron, MCP, and the non-stream path.
- **Registry (single source of truth):** `credentials/custom_models.json` — gitignored (survives pulls), 0600, atomic writes, corrupt files quarantined to `.corrupt-<ts>`. Read FRESH per request — adding/editing a server in the wizard takes effect with NO restart. Module: `Orchestrator/onboarding/custom_servers.py` (`resolve_model`, `qualify`, `window_guard_tokens`, wizard-guidance message constants).
- **Wizard:** the api_keys step has a "Custom model servers" section — "+ Add server" rows (alias + base URL + API key, keyless OK), Validate & Add probes `GET {base_url}/models` (zero-token, cold-safe) and persists discovered models. Endpoints: `GET/POST /onboarding/custom-servers`, `PATCH/DELETE /onboarding/custom-servers/{id}`, `POST /onboarding/validate` with `provider="custom"`. PATCHing base_url clears validation stamps (re-validate after re-pointing).
- **Model ids are qualified:** `<alias>::<model>` (e.g. `gemma-box::gemma-26b`); backend splits on the FIRST `::` to route; empty/`Auto` resolves to the first enabled server's first discovered model. Aliases can never contain `::`.
- **Catalog:** `GET /models/custom` — live-merged from every enabled server on EVERY call (never cached at any layer; new downloads appear immediately). Additive per-model fields: `server` (alias; drives Portal optgroups) and `status` (`loaded`/`unloaded` from llama-swap; renders 🟠 in the Portal dropdown, orange dot on Android).
- **Chat:** `stream_custom_with_reasoning` + `call_custom` in `chat_routes.py` (cloned from the xai pair): full ToolVault tool calling via the BlackBoxToolExecutor catch-all, `reasoning_content` → thinking channel, vision data-URLs, bearer-only-when-key, no max_tokens (reasoning models need headroom), 300s/200s timeouts (cold loads take 15-25s; llama-swap holds the request), one-time `stream_options` 400-retry for old builds.
- **Context guard:** per-server `context_tokens` (default 32768) → `window_guard_tokens(server)` = max(4000, 60%) threaded through BOTH /chat/stream routes AND the tasks.py non-stream fossil block (shared drop ladder `drop_snapshots_to_budget` in `context_builder.py`) — a 240K default context can never overflow a 32K llama.cpp window.
- **Gotchas:** one model resident per llama-swap box (requests queue+serialize; don't ping-pong models); audio/video attachments are unsupported on custom (media auto-route exempts custom; audio parts become text notes); cron jobs on an empty registry fail loudly with wizard guidance (never silently run on Gemini).
- **Plan/design:** `docs/plans/2026-07-08-custom-model-providers.md`.
