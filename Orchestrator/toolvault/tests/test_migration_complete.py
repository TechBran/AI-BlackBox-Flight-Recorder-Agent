...
* exactly 48 module folders, each with a ``schema.json`` (all non-UGV tools,
  INCLUDING ``get_current_operator``),
* zero ``ugv_*`` folders,
* every module validates clean against ``schema_spec``,
* the folder-name set equals the non-UGV tool-name set (no subtraction),
...
def test_exactly_48_folders_each_with_schema():
    folders = _module_folders()
    assert len(folders) == EXPECTED_COUNT, (
        f"expected {EXPECTED_COUNT} module folders, got {len(folders)}"
    )
...
def test_registry_load_canonical_48_no_errors():
    registry.invalidate_cache()
    canonical = registry.load_canonical()
    assert len(canonical) == EXPECTED_COUNT, (
        f"load_canonical returned {len(canonical)}, expected {EXPECTED_COUNT}"
    )
