...
## M7 — MCP ONBOARDING STEP (added 2026-06-27, Brandon)
Goal: a customer-facing onboarding step where an operator can self-serve their MCP connection details — the **public URL + their token + ready-to-paste per-app configs** — so they add the BlackBox to Claude Code/Codex/Antigravity/Desktop when ready. (Today the token is generated by hand + the URL is buried; this surfaces both.)

**Backend — token-management API** (operator-scoped; source of truth = the gitignored `Manifest/mcp_tokens.json` from M3):
- `GET /mcp/connection?operator=<op>` → `{public_url, has_token, token?, per_app_configs}` where per_app_configs are the paste-ready snippets (Claude Code / Codex / Antigravity / Desktop) with the URL + token pre-filled. public_url from the Funnel config or a configured `BLACKBOX_MCP_PUBLIC_URL`.
- `POST /mcp/tokens` (operator) → generate a high-entropy `bbmcp_` token, write `{token: operator}` to the store, return it ONCE. Reuse the M3 store format.
- `DELETE /mcp/tokens` (by token-id / operator) → revoke (remove from the store; takes effect on the live HTTP server's next read — note token map loads at startup, so add hot-reload OR document a restart).
- **Decision:** show the stored token on demand (convenient — the store holds the raw token) vs. store-only-a-hash + show-once-on-generation (more secure, but the user can't re-view it). Pick per the onboarding security bar.

**UI — onboarding "MCP Tool Server" card** (the hub-first console step; see project_onboarding_uplift):
- Shows the public URL, the operator's token (generate / regenerate / revoke + copy, masked until revealed), and the per-app config snippets with copy buttons. Optional "Test connection" affordance.
- 3 surfaces: Portal + Android + WebView (frontend-three-surfaces rule).

**Security:** operator-scoped (never expose another operator's token via the connection endpoint — bind it to the requesting operator); the OAuth/web path connects via the M6 consent gate (URL only, no token in the config). Integrates with the onboarding uplift console rather than a standalone page.

**Sequencing:** build after M6 (OAuth) lands + verifies. Depends on the live token store + the public URL being stable.
