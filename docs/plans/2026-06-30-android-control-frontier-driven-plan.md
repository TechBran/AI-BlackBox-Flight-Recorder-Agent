...
| 3.7 | Device-management API + Portal System-Menu "Devices" | `Orchestrator/routes/device_routes.py` (a general `/devices` API, reused by onboarding + system menu), `Portal/` System-Menu view (+ `Portal/onboarding/*` reuses it) | a **"Devices" section in the Portal System Menu** (not onboarding-only) listing **all Tailscale-net devices** (join `tailscale status` × attestation registry): per device show name/type/status, an **operator↔device assignment** control, a **primary** toggle, and a **default-provider** picker (Gemma opt-in). Endpoints: `GET /devices` (all tailnet nodes + ownership), `POST /devices/{id}/operator`, `.../primary`, `.../default-provider`. Onboarding hub renders the SAME component. | System Menu + onboarding both list all devices; operator-assign persists; set-primary clears old; provider persists; operator-isolated |
| 3.8 | Android MVP System-Menu "Devices" (3-surface parity) | Android `.../ui/settings/` (system/hamburger menu) | mirror the Portal Devices view in the Android MVP System Menu: list all tailnet devices, assign operator↔device, pick primary + provider, consuming the same `/devices` API. Keep the catalog contract additive ([[feedback_frontend_three_surfaces]]). | Android system menu lists devices; assignment/primary/provider round-trip through `/devices`; matches Portal |
