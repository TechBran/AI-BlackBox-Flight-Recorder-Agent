...
8. **Device/provider management = managed-state onboarding flow** (Brandon). Per-operator devices are configured in the managed-state onboarding/hub console (reuse the onboarding-uplift `?mode=manage` hub, not a new surface): the operator designates a **primary device** (the source of truth for the "non-device origin → operator's primary device" routing fallback), sets a **per-device default provider** (Gemma is an explicit opt-in choice here), and can **add/edit devices** for their operator later. **Placement (Brandon refinement):** device management is NOT onboarding-only — it lives as a **"Devices" section in the System Menu of BOTH the main Portal UI and the Android MVP** (the [[feedback_frontend_three_surfaces]] pattern), showing **all devices on the Tailscale net** with an **operator↔device assignment** control (pick which operator owns which device) plus primary + per-device provider. The onboarding flow reuses the SAME surface/endpoints. So device management is always-reachable, not just a first-run step; the registry view joins `tailscale status` (all tailnet nodes) with the attestation registry (operator ownership). Accepted with this: **streaming transport** (SSE/WebSocket over 8765, replacing 2s polling) and **coordinate-adapter sequencing** (Gemini 0-999 for v1; Anthropic/OpenAI absolute-px adapter when those providers join).
