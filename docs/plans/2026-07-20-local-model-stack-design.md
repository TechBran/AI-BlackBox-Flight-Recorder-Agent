...
**Date:** 2026-07-20
**Status:** SPEC v2.1 — hardened by 6-dimension Opus audit 2026-07-20 (see §14); **decisions D1–D12 locked** (Q10–Q15 answered by Brandon 2026-07-20). Remaining open items (Q1–Q9) are implementation-level. **Ready for the implementation plan (superpowers:writing-plans).**
...
| D11 | CU live view *(answers Q14)* | **One shared live view, any BlackBox user may watch.** When a CU session is active, an **in-use flag/indicator** is shown to other users (Portal + Android). No per-operator gating |
...
- **Live view:** noVNC/websockify per session, reverse-proxied by the Orchestrator (`/cu/view/{session_id}` WS). Auth = the Tailscale perimeter (by design, no app-layer auth) — **DECIDED 2026-07-20 (D11): one shared live view, any BlackBox user may watch; no per-operator gating.** Requirement: while a CU session is active, an **in-use flag/indicator** is surfaced to other users in the Portal and the Android MVP so nobody is surprised the display is occupied. noVNC/websockify is the **only genuinely-new piece** (the Xvfb/openbox/x11vnc trio already runs under the real sandbox today) — smoke-check it under the unit (loopback bind is allowed by `RestrictAddressFamilies=AF_INET`; pin the dependency on `PrivateTmp=false`, or move the X11 socket under `/run/user`, so future hardening doesn't silently break CU). Portal gets a live-view panel (iframe pattern like the zellij terminal); Android renders the same page in a WebView (three-surfaces rule satisfied).
...
- **MS02's snapshot Volume is a transplant and does not belong there.** The dev box's corpus was copied over purely to stress-test the embeddings layer — Brandon ran a giant-corpus re-embed through the GPU-served 8B and **it worked great** (a strong G1 throughput/feasibility signal). Per Brandon's D8 decision (2026-07-20), **only the transplanted snapshot ledger is removed** — operators, settings, and credentials on MS02 stay as they are. **Phase 2 begins with the snapshot-data reset (below).** This transplant pain is also the motivation for the parked export/import workstream (§12).
...
14. **Q14 — CU live-view auth isolation:** ~~per-operator gating?~~ **ANSWERED 2026-07-20 → D11.** One shared live view — any BlackBox user may watch; a visible **in-use flag** tells other users a CU session is active (Portal + Android). No per-operator gating.
